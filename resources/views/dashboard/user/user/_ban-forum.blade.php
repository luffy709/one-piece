<x-dashboard-card :title="$user->name" page="user" :model="$user">
    <div class="dashboard-card__row">
        ...
        <div class="dashboard-card__col">
            <form action="{{ route('dashboard.user.ban_chat', $user) }}" method="post">
                @csrf
                @method('delete')

                <button type="submit" class="dashboard-card__button dashboard-user__body__users__button--ban-chat">Ban chat</button>
            </form>
        </div>
    </div>
